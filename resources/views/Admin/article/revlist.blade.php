        ...
        <table class="table table-hover">
            ...
            @foreach($articles as $article)
                <tr>
                    ...
                    <td>
                        <a href="#" class="btn btn-success btn-sm btn-del" title="恢复的文章可以在回收站恢复">撤销</a>
                        <form onsubmit="return confirm('您是否确定要恢复该文章？{{ $article->title }}')" action="{{ url('admin/recover',$article->id) }}" method="post" id="delete_sub">
                            {{csrf_field()}}
                        </form>
